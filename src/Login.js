...
.then((res) => res.json())
//.then((res)=>res.error===true?console.log(true):console.log(jwt.decode(res.token).email))
.then((res)=>res.error===true?setMsg({style:"warning", info: res.message}):localStorage.setItem("token",jwt.decode(res.token)),setMsg({style:"success", info: "Login success"}))        }}
